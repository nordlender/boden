...
- **`hasUnpaidFees` and `userIsMember` came back `null`** in the raw response,
  ...
  caching on our end — every login makes a fresh live call to bloc). **Not
  resolved** — current working assumption (the user's explicit call) is that
  it's stale data on bloc's end, to revisit later. Worth checking bloc's other
  granted method, `api/account/listmypersonprofiles` (per
...
1. **Resolve the `hasUnpaidFees`/`userIsMember` null question.** Try the
   `api/account/listmypersonprofiles` method, or just wait and retest — either
   way this blocks the moderator-review-page fields from ever showing real
   data.
2. **Remove the temporary debug log** in `src/auth.ts` once #1 is settled.

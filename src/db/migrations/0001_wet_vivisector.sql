...
-- No pre-existing order carries real dates (this column didn't exist before
-- this migration) — '1970-01-01' is a placeholder for dev-era rows only,
-- never expected to be hit against a real production database.
INSERT INTO `__new_orders`("id", "order_code", "user_id", "status", "from_date", "to_date", "note", "confirmed_by_user_id", "returned_by_user_id", "created_at", "activated_at", "returned_at", "rejected_at", "rejected_reason") SELECT "id", "order_code", "user_id", "status", '1970-01-01', '1970-01-01', "note", "confirmed_by_user_id", "returned_by_user_id", "created_at", "activated_at", "returned_at", "rejected_at", "rejected_reason" FROM `orders`;--> statement-breakpoint
